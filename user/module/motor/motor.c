 ...
 *
 * 可调范围：0.0 ~ 8.0
 * 当前值：0.0（V6.9关闭：扑动系统目标周期性变化，前馈反而导致超调）
 *
 * 调整建议：
 *   - 扑动系统（周期性目标）→ 设为0.0（关闭前馈）
 *   - 阶跃响应系统（突变目标）→ 设为2.0-3.5
 *   - 出现超调（实际>目标）→ 设为0.0
 *   - 跟踪滞后严重 → 尝试1.0-2.0
 *
 * ⚠️ V6.9重要发现：
 *   扑动系统的目标角度是周期性平滑变化的（波形表生成），
 *   不是突变信号。前馈会持续输出，导致实际角度超过目标，
 *   造成超调。纯PID控制更适合周期性扑动！
 *
 * 原理：
 *   前馈输出 = 目标角度变化量 × FEEDFORWARD_GAIN
 *   扑动系统：目标每步都变（4°/步），前馈持续叠加 → 超调
 *   建议：关闭前馈，只用PID：总输出 = PID(误差)
 */
